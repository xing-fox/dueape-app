...
export default {
  ...
  methods: {
    draw (speed) {
      ...
      // 曲线绘制
      this.xOffset += speed
      this.drawWave(ctx, this.xOffset)
      this.drawWave(ctx, this.xOffset*1.4)
      this.drawWave(ctx, this.xOffset*1.8)
      requestAnimationFrame(this.draw.bind(this, speed))
    },
    drawWave (ctx, xOffset = 0, waveWidth = 0.015, waveHeight = 20) {
      const points = [];
